...
> ⚠️ **STALE 2026-05-18** — This doc was generated by Manus (initial scaffold) and has NEVER been updated.
> 5 known lies vs current code:
>   1. **Auth**: claims Manus OAuth — actual = password auth (commit `f547e22`)
>   2. **Hosting**: claims Manus hosting — actual = local Docker + ngrok tunnel
>   3. **Booking**: claims Calendly integration — REMOVED (commit `e7e38a4`), native booking shipped (`59a5a95`)
>   4. **DB**: claims TiDB option — actual = plain MySQL via Drizzle ORM only
>   5. **Output**: claims PPTX — actual = PDF only (commit `f10daeb`)
> **CANONICAL source of truth**: `~/.claude/projects/-home-arthur/memory/tiragesmarie-project.md`
> Do NOT use this file as authoritative. Rewrite or archive pending Arthur review.
